...
ProductPolicy
ProductPolicyParser::FromBinary(const std::vector<uint8_t> &Binary)
{
    ...
    while (pb < EndOfpb)
    {
        ...
        case REG_SZ: {
            ...
            PolicyVal.GetData<PolicyValue::TypeOfString>().assign(
                AsPtrOf<wchar_t>(pValData), pVal->DataSize / sizeof(wchar_t));
        ...
        case REG_BINARY: {
            ...
            PolicyVal.Name.assign(pValName, pVal->NameSize / 2);
            PolicyVal.GetData<PolicyValue::TypeOfBinary>().assign(
                AsPtrOf<uint8_t>(pValData), AsPtrOf<uint8_t>(pValData) + pVal->DataSize);
            PolicyVal.Flags = pVal->Flags;
            PolicyVal.Reserved = pVal->Reserved;
...
std::vector<uint8_t>
ProductPolicyParser::ToBinary(const ProductPolicy &PolicyObject)
{
    ...
    for (const PolicyValue &Policy : PolicyObject._Policies)
    {
        ...
        BinaryValue.NameSize = static_cast<uint16_t>(Policy.GetName().length() * sizeof(wchar_t));
        BinaryValue.DataType = static_cast<uint16_t>(Policy.GetType());
        ...
        case PolicyValue::TypeLabel::Binary:
            BinaryValue.DataSize = static_cast<uint16_t>(Policy.GetData<PolicyValue::TypeOfBinary>().size());
            break;
        case PolicyValue::TypeLabel::UInt32:
            BinaryValue.DataSize = sizeof(Policy.GetData<PolicyValue::TypeOfUInt32>());
            break;
        default:
            throw std::invalid_argument("Unexpected value type.");
        }
