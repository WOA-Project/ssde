...
size_t
ProductPolicy::NumberOfPolicies() const noexcept
{
    return _Policies.size();
}

size_t
ProductPolicy::FindPolicy(const std::wstring &Regex, size_t StartPos)
{
...
PolicyValue &
ProductPolicy::operator[](size_t Index)
{
    return _Policies[Index];
}

PolicyValue &
ProductPolicy::operator[](const std::wstring &Name)
{
...
const PolicyValue &
ProductPolicy::operator[](size_t Index) const
{
    return _Policies[Index];
}

const PolicyValue &
ProductPolicy::operator[](const std::wstring &Name) const
{
    size_t i = 0, j = _Policies.size();
    while (true)
    {
